define(["filter_pycode/little-loader-lazy"],/*['codemirror', 'skulpt', 'terminal', 'split'],*/ 

	//function (cm, sk, Terminal, split) {
	function (lload) {
	...
	function pyCraft(pycodes) {
		...
		if (!pySkulptLoaded || !pyTerminalLoaded || !pySplitLoaded)
			return;
		...
		for (var i = 0, len = pycodes.length; i < len; i++) 
		{
			...
			var rawcode = pyelem.innerText.replace(/^\s+|\s+$/g, '');
			...
			var cmeditor = window.CodeMirror(document.getElementById(editorid), {
				value: rawcode,
				lineNumbers: true,
				mode:  "python"
			});
			...
			btnrun.onclick = function () {  
											...
											Sk.configure({output: function(text) 
																			{ 
																				Sk.outputBuffer += text;
																				if (text.indexOf("\n") != -1)
																				{
																					pyTerminal.print(Sk.outputBuffer);	
																					Sk.outputBuffer = "";
																				}
			...
			var splitOptions = {
				...
				onDragEnd: function() { 
													...
													if (this.output.offsetHeight < 100)
													{
														...
														{
															this.container.style.height = Math.min(this.container.offsetHeight+diff, 800) + "px";
														}
			...
			btnfull.style = "position: absolute; right: 0px; margin-right: 20px";
			...
			btnfull.onclick = function () {  
									if (this.pycode.className.includes("pycode full"))
									{
										...
										this.splitOptions.container.style.height = this.splitOptions.originalHeight + "px";
	...
	function pyCode ()
	{
		if (pyDocReady && !pyInitialized)
		{
			var $pycodes = document.getElementsByClassName('pycode'); 

			if ($pycodes.length > 0)
			{
				scriptFolder = "/filter/pycode/amd/src/";
				
				/*
				window._lload(scriptFolder + "lib/codemirror/lib/codemirror.js", function (err) { 
						window._lload(scriptFolder + "lib/codemirror/mode/python/python.js", function (err) { pyCodeMirrorLoaded = true; pyCraft($pycodes); });
					});
				*/
				...
				docReady(function() 
					{
							...
							require.config({
								packages: [{
								  name: "codemirror",
								  location: "/filter/pycode/amd/src/lib/codemirror/",
								  main: "lib/codemirror"
								}]
							  });
							require(["codemirror", "codemirror/mode/python/python",
